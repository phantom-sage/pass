...
@section('content')
    ...
    <section class="mt-5 py-2">
        <div class="container mx-auto">
            <h2 class="mx-3 py-5 text-lg md:text-3xl inline @if(app()->getLocale() === 'en') float-left @elseif(app()->getLocale() === 'ar') float-right @endif">{{ __('contactuspage.passContactUsInfo') }}</h2>
            <h4 class="mx-3 py-5 text-lg md:text-3xl @if(app()->getLocale() === 'en') float-right @elseif(app()->getLocale() === 'ar') float-left @endif inline">{{ __('contactuspage.followUs') }}</h4><div class="clearfix"></div>
            <div class="mx-3 flex flex-wrap">
                ...
                <div class="w-3/12 @if(app()->getLocale() === 'en') text-right @elseif(app()->getLocale() === 'ar') text-left @endif">
                    <span class="p-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-6 w-6 text-gray-500 inline">
                            <path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6.066 9.645c.183 4.04-2.83 8.544-8.164 8.544-1.622 0-3.131-.476-4.402-1.291 1.524.18 3.045-.244 4.252-1.189-1.256-.023-2.317-.854-2.684-1.995.451.086.895.061 1.298-.049-1.381-.278-2.335-1.522-2.304-2.853.388.215.83.344 1.301.359-1.279-.855-1.641-2.544-.889-3.835 1.416 1.738 3.533 2.881 5.92 3.001-.419-1.796.944-3.527 2.799-3.527.825 0 1.572.349 2.096.907.654-.128 1.27-.368 1.824-.697-.215.671-.67 1.233-1.263 1.589.581-.07 1.135-.224 1.649-.453-.384.578-.87 1.084-1.433 1.489z"/>
                        </svg>
