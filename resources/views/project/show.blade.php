            ...
            <div class="w-full">
                <div class="rounded shadow-sm border my-5 relative">
                    ...
                    <span class="absolute top-3 right-3 bg-white px-5 rounded-lg">
                        <form id="projectLikeForm" action="{{ route('saveProjectLike', ['locale' => app()->getLocale(), 'project' => $project]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <a href="{{ route('saveProjectLike', ['locale' => app()->getLocale(), 'project' => $project]) }}" onclick="event.preventDefault(); this.closest('form').submit(); this.disabled = true;">
