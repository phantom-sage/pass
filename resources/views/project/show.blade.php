    ...
    <div class="container mx-auto">
        ...
        @if(session('commentSaveStatus'))
            <div x-data="{ open: true }" x-show="open" class="my-5 shadow-lg bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
                ...
                <span class="absolute top-0 bottom-0 @if(app()->getLocale() === 'en') right-0 @elseif(app()->getLocale() === 'ar') left-0 @endif px-4 py-3">
        ...
        @if(count($project->comments) > 0)
            <div class="flex flex-wrap">
                <ul class="w-full ml-4 my-4">
                    @foreach($project->parentComments as $comment)
                ...
                <div class="w-full ml-4 my-4">
                    <form action="{{ route('saveProjectComment', ['locale' => app()->getLocale(), 'project' => $project]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <div class="w-full mx-3">
                                <textarea name="body" class="p-3 placeholder:text-gray-700 w-full border rounded" required placeholder="@if(app()->getLocale() === 'en') Enter your comments here @elseif(app()->getLocale() === 'ar') اكتب الكومنت هنا... @endif"></textarea>
                                <small class="text-red-700">
                                @error('body')
                                    {{ $message }}
                                @enderror
